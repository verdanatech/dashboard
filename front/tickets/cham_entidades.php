...
<head>
	...
	if ($ontem['total'] > $hoje['total']) {
		$up_down = "../img/down.png";
	}
	if ($ontem['total'] < $hoje['total']) {
		$up_down = "../img/up.png";
	}
	if ($ontem['total'] == $hoje['total']) {
		$up_down = "../img/blank.gif";
	}
	...
	<div id='container-fluid'>
		<div id="head-cham" class="row-fluid">
			<table class="col-ms-12 col-sm-12" style="margin-left: auto; margin-right: auto;">
				...
				<tr>
					<td align="center"><span class="titulo_cham"><?php echo  $ent_name; ?></span> </td>
				</tr>
				<tr>
					<td align="center">
						...
						<span class="titulo_cham"><a href="cham_entidades.php?ent=<?php echo $ent; ?>"> <?php echo "/ " . __('Today', 'dashboard'); ?>: </a>
							...
							<img src=<?php echo $up_down; ?> class="up_down" alt="" style="margin-top: -10px;" title=<?php echo __('Yesterday', 'dashboard') . ':';
																														echo $ontem['total']; ?>> </span>
					</td>
				</tr>
			</table>
			<p></p>
		...
		<div id="lista_chamados" class="well info_box row-fluid report" style="">
			...
			if (isset($_REQUEST['order'])) {
				...
				switch ($order1) {
					case "td":
						$order = "ORDER BY glpi_tickets.id DESC";
						break;
					case "ta":
						$order = "ORDER BY glpi_tickets.id ASC";
						break;
					case "sd":
						$order = "ORDER BY glpi_tickets.status DESC, glpi_tickets.id ASC";
						break;
					case "sa":
						$order = "ORDER BY glpi_tickets.status ASC, glpi_tickets.id ASC";
						break;
					case "tid":
						$order = "ORDER BY glpi_tickets.name DESC";
						break;
					case "tia":
						$order = "ORDER BY glpi_tickets.name ASC";
						break;
					case "ted":
						$order = "ORDER BY glpi_tickets.id DESC";
						break;
					case "tea":
						...
						break;
					case "pd":
						$order = "ORDER BY glpi_tickets.priority DESC, glpi_tickets.date ASC";
						break;
					case "pa":
						$order = "ORDER BY glpi_tickets.priority ASC, glpi_tickets.date ASC";
						break;
					case "dd":
						$order = "ORDER BY glpi_tickets.time_to_resolve DESC";
						break;
					case "da":
						$order = "ORDER BY glpi_tickets.time_to_resolve ASC";
						break;
				}
			} else {
			...
			//select tickets				
			$sql_cham = "SELECT DISTINCT glpi_tickets.id AS id, glpi_tickets.name AS descri, glpi_tickets.status AS status, glpi_tickets.date_mod, 
		...
		<thead>
			...
			echo "<th style='text-align:center;'> Grupo Resolvedor </th>
		...
		<tbody>";
			...
			while ($row = $DB->fetch_assoc($result_cham)) {
				...
				if ($status1 == "14") {
					$status1 = "waiting_list";
				}
				if ($status1 == "15") {
					$status1 = "in_attendance";
				}
		...
		AND glpi_tickets.entities_id = " . $ent . " ";
				...
				WHERE t.id =" . $row['id'] . "
				AND gt.type =2";
				$result_tecgrup = $DB->query($sql_tecgrup);
				$row_group = $DB->fetch_assoc($result_tecgrup);
		...
		<tr class='title' style='font-weight:normal;'>
			...
			<td style='vertical-align:middle;'><span >" . $row_req['name'] . " " . $row_req['sname'] . "</span> </td>";
				...
				if ($show_due != 0) {
					if ($count_due > 0) {
						...
						if ($barra > 95 and $barra < 100) {
							$cor = "progress-bar-danger";
						}
						if ($barra > 70 and $barra < 95) {
							$cor = "progress-bar-warning";
						}
						if ($barra > 50 and $barra <= 70) {
							$cor = "progress-bar-default";
						}
			...
			<td  style='vertical-align:middle; text-align:center;'><a href=../../../../front/ticket.form.php?id=" . $row['id'] . " target=_blank >" . $row_group['name'] . "</a></td>
			</tr>";
			}
