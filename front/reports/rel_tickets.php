...
<body style="background-color: #e5e5e5;">
	...
	<div id='content'>
		<div id='container-fluid' style="margin: <?php echo margins(); ?> ;">
			<div id="charts" class="fluid chart">
				<div id="head-lg" class="fluid" style="min-height: 390px;">
			...
			ORDER BY cname ASC ";
										...
										if (isset($_REQUEST["sel_req"]) && $_REQUEST["sel_req"] != '0') {
											$id_req = $_REQUEST["sel_req"];
										} else {
											$id_req = '';
										}

										if (isset($_REQUEST["sel_pri"]) && $_REQUEST["sel_pri"] != '0') {
											$id_pri = $_REQUEST["sel_pri"];
										} else {
											$id_pri = '';
										}

										if (isset($_REQUEST["sel_cat"]) && $_REQUEST["sel_cat"] != '0') {
											$id_cat = $_REQUEST["sel_cat"];
										} else {
											$id_cat = '';
										}

										if (isset($_REQUEST["sel_resolvedor"]) && $_REQUEST["sel_resolvedor"] != '0') {
											$id_resolvedor = $_REQUEST["sel_resolvedor"];
										} else {
											$id_resolvedor = '';
										}
										if (isset($_REQUEST["sel_operacao"]) && $_REQUEST["sel_operacao"] != '0') {
											$id_operacao = $_REQUEST["sel_operacao"];
										} else {
											$id_operacao = '';
										}
										if (isset($_REQUEST["sel_localizacao"]) && $_REQUEST["sel_localizacao"] != '0') {
											$id_localizacao = $_REQUEST["sel_localizacao"];
										} else {
											$id_localizacao = '';
										}
			...
			ORDER BY id ASC ";
								...
								<tr>
									...
									<td style="margin-top:2px;">
										...
										$name = 'sel_localizacao';
										$options = $arr_localizacao;
										$selected = $id_localizacao;
	...
	<tbody>";
						...
						while ($row = $DB->fetch_assoc($result_cham)) {
							...
							if ($prio == "1") {
								$pri = _x('priority', 'Very low');
							}
							if ($prio == "2") {
								$pri = _x('priority', 'Low');
							}
							if ($prio == "3") {
								$pri = _x('priority', 'Medium');
							}
							if ($prio == "4") {
								$pri = _x('priority', 'High');
							}
							if ($prio == "5") {
								$pri = _x('priority', 'Very high');
							}
							if ($prio == "6") {
								$pri = _x('priority', 'Major');
							}
	...
	<tr></tr>
				...
				<script type='text/javascript'>
					...
					$(document).ready(function() {
						$("#sel_resolvedor").select2({
							dropdownAutoWidth: true
						});
					});
					$(document).ready(function() {
						$("#sel_operacao").select2({
							dropdownAutoWidth: true
						});
					});
					$(document).ready(function() {
