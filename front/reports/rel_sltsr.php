...
if (!isset($_POST["sel_gr"])) {
	$id_grupo = $_POST["sel_gr"];
	$id_group = implode(", ", $id_grupo);
	$id_gr = "($id_group)";
} else {
	$id_grupo = $_POST["sel_gr"];
	$id_group = implode(", ", $id_grupo);
				...
				<div id="pad-wrapper">
					<div id="head-lg" class="fluid">
						...
						ORDER BY name ASC ";
									...
									<tr>
										<td height="12px" style="margin-top: 15px;"></td>
										<td style="margin-top:10px;margin-left:240px;">
				...
				if ($con == "1") {
					...
					if ($id_gr != "(0)") {
						...
						$sql_cham =
							"SELECT glpi_tickets.id AS id, glpi_tickets.name AS descr, glpi_tickets.date AS date, glpi_tickets.solvedate as solvedate,
									...
									FROM glpi_tickets
									LEFT JOIN glpi_groups_tickets as gt ON (glpi_tickets.id = gt.tickets_id)
									WHERE glpi_tickets.is_deleted = 0
									" . $slaid . "
									AND gt.groups_id IN " .   $id_gr . " 
					...
					if ($conta_cons > 0) {
						...
						<table class='col-md-12 col-sm-12 fluid'  style='font-size: 18px; font-weight:bold;' cellpadding = 1px >
							<tr>
								<td style='font-size: 16px; font-weight:bold; vertical-align:middle;'><span style='color:#000;'>" . __('SLA') . ": </span>" . "" . $ent_name['name'] . "</a></td>
